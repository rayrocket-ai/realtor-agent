...
| Agent | Skill | Status |
|---|---|---|
| Booking agent | `/book-showing` | Working — Realm step pending access (see `docs/integrations.md`) |
...
The agent looks up clients/listings in Airtable, checks the "Real Estate" Google Calendar for conflicts, confirms the exact slot with you, then creates the calendar event, logs the showing in the CRM table, and drafts the client confirmation email. The Realm booking itself is output as a paste-ready block until Realm access is connected.
